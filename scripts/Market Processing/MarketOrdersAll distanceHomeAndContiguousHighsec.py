...
def update_jumps_from_home():
    config = load_config()
    home_system_name = config["homeSystem"]
    ...
    with sqlite3.connect(config["dbPath"]) as conn:
        ...
        # Update all records with the appropriate jumpsFromHome
        for system_id, jumps in system_jumps.items():
            cursor.execute("UPDATE MarketOrdersAll SET jumpsFromHome = ? WHERE solarSystemID = ?", (jumps, system_id))
