...
def update_jumps_from_home():
    ...
    with sqlite3.connect(config["dbPath"]) as conn:
        ...
        cursor.execute("SELECT solarSystemID FROM SolarSystems WHERE solarSystemName = ?", (jita,))
        result = cursor.fetchone()
        if not result:
            raise ValueError(f"Trade hub '{jita}' not found in SolarSystems table.")
        jitaID = result[0]
        ...
        for system_id in unique_system_ids:
            if system_id is None:
                continue
            try:
                jumps = find_jumps_between_system_ids(config["dbPath"], jitaID, system_id, 0.45) #highsec
                if jumps is None:
                    jumps = find_jumps_between_system_ids(config["dbPath"], jitaID, system_id, -2.0)
            except Exception as e:
                print(f"Error finding jumps between {jitaID} and {system_id}: {e}")
                jumps = -1  # Use -1 to indicate error/unreachable

            system_jumps[system_id] = jumps
            #print(jumps)

        # Update all records with the appropriate distanceJita
        for system_id, jumps in system_jumps.items():
            cursor.execute("UPDATE MarketOrdersAll SET distanceJita = ? WHERE solarSystemID = ?", (jumps, system_id))
